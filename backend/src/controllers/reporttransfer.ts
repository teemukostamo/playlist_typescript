...
import { Report_Transfer } from '../models/Report_Transfer';
...
// @route   GET /:filename
// @access  Private
export const sendFileToClient = asyncHandler((req: Request, res: Response) => {
  res.download(path.join(__dirname, `../transfers/${req.params.filename}`));
});

interface TrackToReport {
  program_date: string;
...
interface StringifiedTrackToReport {
  program_date?: string;
  helper_field?: string;
  copyright_holder_1?: string;
  copyright_holder_2?: string;
  copyright_holder_3?: string;
  copyright_holder_4?: string;
  copyright_holder_5?: string;
  times?: string;
  type?: string;
  jingle?: string;
  comment?: string;
...
export const generateTransferFile = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (arrayWithTracks) {
      ...
      const newArr: Array<StringifiedTrackToReport> = [];
      ...
      arrayWithTracks.forEach(track => {
        ...
        console.log(
          'new program end time length',
          new_programm_end_time.length
        );
